...
object AppManualModule {
    ...
    private fun providesUserDao(context: Context): UserDao =
        UserDb.getDbInstance(context).getUserDao()

    private fun providesUserRepository(context: Context): UserContract =
        UserRepo(providesUserDao(context))

    fun providesSaveUserUseCase(context: Context): SaveUserUseCase = SaveUserUseCase(
        providesUserRepository(context)
    )

    fun providesRetrieveSavedUsersUseCase(context: Context): RetrieveSavedUsersUseCase =
        RetrieveSavedUsersUseCase(providesUserRepository(context))
}